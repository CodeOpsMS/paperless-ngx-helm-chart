...
from 2.20.15 to 3.1.2. Chart `0.3.23` remains the stable default and normal Helm
...
- Upgrade directly to `3.1.2`; do not stop on the broken `3.0.1` migration.
...
- Update API clients to version 9 or 10; Paperless 3 no longer supports API
  ...
  API version 10.
- Remove `PAPERLESS_SEARCH_LANGUAGE` for this 3.1.2 release. Explicitly setting
...
- Ensure the data PVC has generous free space and monitor Tantivy index growth.
  Paperless 3.1.2 can retain orphaned Tantivy segments when several processes
...
- Review workflow actions that assign custom-field values. Paperless 3.1.2 can
...
- Check amd64 nodes for the `sse4_2` CPU flag and move the workload to nodes
  ...
  training is not sufficient for the official 3.1.2 image: document consumption
...
- Rebuild and retest custom images and native extensions. Paperless 3 requires
  Python 3.11 or newer; the official 3.1.2 image uses Python 3.12 on Debian
  Trixie.
- If an external Tika service is configured, pin `apache/tika:3.3.1.0`; do not
  ...
  with Paperless 3.1.2 (see upstream
...
| `env.PAPERLESS_SEARCH_LANGUAGE` | Remove it for Paperless 3.1.2; the schema rejects this known startup-breaking setting |
...
- `/api/status/` reports Paperless 3.1.2, PostgreSQL status `OK`, no unapplied
